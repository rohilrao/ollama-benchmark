...
METRICS = ("wall_time_sec", "ttft_sec", "tokens_per_sec", "batch_tokens_per_sec")
...
class MultiModelBenchmark(OllamaBenchmarkBase):
    ...
    def _placement(self, bd: dict, load_sec: dict, name: str) -> dict:
        ...
        return {"load_sec": load_sec.get(name),
                "vram_mb": bd.get("vram_mb"), "total_mb": bd.get("total_mb"),
                "cpu_mb": bd.get("cpu_mb"), "gpu_pct": bd.get("gpu_pct"),
    ...
    async def measure_point(self, configs: list) -> dict:
        ...
        Returns {status, error, elapsed_sec, vram_total_mb, cpu_total_mb,
        ...
        for cfg in configs:
            ...
            try:
                ...
                return {"status": status, "error": f"load_failed[{cfg['model']}]: {msg}",
                        "elapsed_sec": None, "vram_total_mb": None, "cpu_total_mb": None,
        ...
        placed = {c["model"]: {**blank[c["model"]],
                  ...
                  for c in configs}
        head = {"elapsed_sec": None, "vram_total_mb": loaded["vram_total_mb"],
                "cpu_total_mb": loaded["cpu_total_mb"], "all_on_gpu": loaded["all_fully_on_gpu"]}
        ...
        if spilled:
            detail = " ".join(f"{m}={p['cpu_mb']:.0f}MB/{p['cpu_pct']}% on CPU"
        ...
        for cfg in configs:
            ...
            if oks:
                ...
                row.update({m: statistics.mean(r[m] for r in oks)
                            for m in ("wall_time_sec", "ttft_sec", "tokens_per_sec")})
        ...
        return {"status": "ok" if not failures else "partial_or_full_failure",
                ...
                "elapsed_sec": elapsed,
                "vram_total_mb": peak["vram_total_mb"],
                "cpu_total_mb": peak["cpu_total_mb"],
    ...
    async def _sample_vram(self, stop: asyncio.Event, baseline: dict) -> dict:
        ...
        while not stop.is_set():
            ...
            try:
                ...
                if bd["vram_total_mb"] > peak["vram_total_mb"]:
                    peak = bd
            except Exception:
            ...
            for rep in range(1, self.reps + 1):
                ...
                for cfg in configs:
                    ...
                    rows.append({
                        ...
                        "vram_mb": pm.get("vram_mb"), "cpu_mb": pm.get("cpu_mb"),
                        "total_mb": pm.get("total_mb"),
                        "gpu_pct": pm.get("gpu_pct"), "cpu_pct": pm.get("cpu_pct"),
                        "vram_total_mb": res["vram_total_mb"],
                        "cpu_total_mb": res["cpu_total_mb"], "all_on_gpu": res["all_on_gpu"],
                ...
                if res["status"] == "ok":
                    detail = "  ".join(
                        f"{c['model']} ttft={res['per_model'][c['model']]['ttft_sec']:.2f}s"
                        for c in configs)
                    self._log(f"  ok  vram_total={res['vram_total_mb']:.0f}MB  "
                              f"elapsed={res['elapsed_sec']:.1f}s  {detail}")

        offload = {r["point_tag"]: r for r in rows if r["point_status"] == "cpu_offload"}
        if offload:
            self._log(f"\n{len(offload)} grid point(s) skipped — model spilled to CPU:")
            for tag in offload:
                for r in (x for x in rows if x["point_tag"] == tag and (x["cpu_mb"] or 0) > 0):
                    self._log(f"  {tag}  {r['model']}: {r['cpu_mb']:.0f}MB of "
                              f"{r['total_mb']:.0f}MB on CPU ({r['cpu_pct']}%)")
    ...
    def plot_vram(self, rows: list, filename: str = "vram.png"):
        ...
        tags, series = self._points(rows, "vram_mb")
        if not tags:
    ...
    def plot_latency(self, rows: list, metric: str = "ttft_sec", filename: str = None):
        ...
        if metric not in METRICS:
            self._log(f"Unknown metric '{metric}'; expected one of {sorted(METRICS)}.")
            return
