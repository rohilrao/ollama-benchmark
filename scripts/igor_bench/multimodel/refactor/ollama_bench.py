...
class OllamaBenchmarkBase:
    ...
    async def ps_breakdown(self, models: list) -> dict:
        ...
        Returns {per_model: {name: {loaded, vram_mb, total_mb, cpu_mb, gpu_pct,
                 cpu_pct, fully_on_gpu}}, vram_total_mb, cpu_total_mb,
        ...
        for name in models:
            ...
            if m is None:
                per_model[name] = {"loaded": False, "vram_mb": 0.0, "total_mb": 0.0, "cpu_mb": 0.0,
                                   "gpu_pct": None, "cpu_pct": None, "fully_on_gpu": None}
                continue
            vram_mb = (getattr(m, "size_vram", 0) or 0) / 1024 ** 2
            total_mb = (getattr(m, "size", 0) or 0) / 1024 ** 2
            cpu_mb = max(total_mb - vram_mb, 0.0)   # the part that spilled to system RAM
            gpu_pct = round(100 * vram_mb / total_mb, 1) if total_mb > 0 else None
            per_model[name] = {"loaded": True, "vram_mb": vram_mb, "total_mb": total_mb,
                               "cpu_mb": cpu_mb, "gpu_pct": gpu_pct,
                               "cpu_pct": None if gpu_pct is None else round(100 - gpu_pct, 1),
                               "fully_on_gpu": None if gpu_pct is None else gpu_pct >= 99.95}
            vram_total += vram_mb
            cpu_total += cpu_mb

        return {"per_model": per_model, "vram_total_mb": vram_total, "cpu_total_mb": cpu_total,
    ...
    async def ps_snapshot(self) -> str:
        ...
        try:
            ...
            return json.dumps([{"model": m.model,
                                "vram_mb": round((getattr(m, "size_vram", 0) or 0) / 1024 ** 2, 1)}
    ...
    async def stream_request(self, model: str, i: int, num_ctx: int = None,
        ...
        """
        One streaming request, returning wall time, TTFT, tok/s and thinking vs
        content chunk counts (reasoning models stream both in the same response).
        ...
        return {"wall_time_sec": wall,
                "ttft_sec": ttft if ttft is not None else wall,
